...
Consideriamo la resa \(Y_1\) di una reazione chimica che dipende da \(2\) fattori: il tempo della reazione \(X_1\) e la temperatura \(X_2\) (cf.(D.C. Montgomery 2013, Ex 11.2)).
...
Il dominio sperimentale con i valori autentici dei fattori, il dominio sperimentale codificato e le risposte sono dati in \autoref{tab:MatrInfoCCD}.
\newpage

\begin{table}

\caption{\label{tab:unnamed-chunk-5}Dominio sperimentale autentico, dominio sperimentale
codificato e risposte\label{es_matr_mod}}
\centering
...
Nell'applicativo, nel menù \emph{CCD} selezioniamo 2 fattori, 2 punti al centro e scegliamo un disegno di tipo sferico, \autoref{fig:ccd2}
\newpage
...
\caption{Scelta numero fattori, punti al centro e tipo di CCD nell'applicativo\label{fig:ccd2}}\label{fig:unnamed-chunk-6}
\end{figure}

Ognuna delle 3 risposte sarà analizzata con il modello quadratico \autoref{fig:ccd3}. Dalla lettura della matrice di dispersione si nota la non ortogonalità del modello (i termini quadratici sono correlati tra di loro, v. riquadro colorato in giallo nella Figura).
...
\caption{Modello e matrice di dispersione del CCD\label{fig:ccd3}}\label{fig:unnamed-chunk-7}
\end{figure}

Le linee di livello del Leverage \autoref{fig:ccd4} mettono in evidenza invece la ruotabilità del piano sperimentale scelto.
...
\caption{Grafico delle linee di livello del Leverage\label{fig:ccd4}}\label{fig:unnamed-chunk-8}
\end{figure}

Procediamo come di consueto inserendo le Risposte nell'apposito riquadro. Una volta convalidato il modello con tre misure indipendenti \autoref{matr_conv}
\newpage

\begin{table}

\caption{\label{tab:unnamed-chunk-9}Misure indipendenti per convalidare il modello\label{matr_conv}}
\centering
...
possiamo utilizzare il grafico delle linee di livello \autoref{fig:ccd5} per confrontare le risposte tra di loro.

\begin{figure}[ht]
\includegraphics[width=0.5\linewidth]{Immagini/CCD/05_liv1} \includegraphics[width=0.5\linewidth]{Immagini/CCD/06_liv2} \includegraphics[width=0.5\linewidth]{Immagini/CCD/07_liv3} \caption{Grafico delle linee di livello delle risposte $Y_1,Y_2$ e $Y_3$\label{fig:ccd5}}\label{fig:unnamed-chunk-10}
\end{figure}

Nell'applicativo si noti che è possibile scegliere il colore delle linee di livello. Questa risorsa è utile quando, come in questo caso, bisogna confrontare più grafici tra loro \autoref{fig:ccd8}.

\begin{figure}[ht]
\includegraphics[width=0.5\linewidth]{Immagini/CCD/08_livelli} \includegraphics[width=0.5\linewidth]{Immagini/CCD/09_livelli} \caption{Grafici delle linee di livello delle risposte $Y_1,Y_2$ e $Y_3$ sovrapposti e regione di ottimo\label{fig:ccd8}}\label{fig:unnamed-chunk-11}
\end{figure}

Analizzando i vincoli e le linee di livello della resa \(Y_1\) si vede che le zona in cui cercare soluzione del problema sono quelle colorate in grigio (v. \autoref{fig:ccd8}).
